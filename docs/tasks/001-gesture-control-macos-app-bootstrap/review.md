...
- 2026-06-16 Phase 13 TDD 和验证：
  ...
  - Phase 13.2 manual preflight confirmation：用户已确认当前 App 已出现摄像头画面、摄像头指示灯已亮起；用户点击 `Recheck` 后系统提示确认“辅助功能”权限，用户已授予该权限。该确认解除 Camera preview 现场阻塞，但随后确认当前 ad-hoc app 未被 TCC 信任，Accessibility runtime trust 仍未解除；该确认也不等同于 Vision gesture matrix 通过。
  - Accessibility TCC code requirement mismatch root cause：用户授予 Accessibility 后 HUD 仍显示红点 `Permission`，不是手势分类失败。`tccd` 日志显示 `Failed to match existing code requirement for subject com.m1ngwym.walkflowmac and service kTCCServiceAccessibility`，并列出旧/新两个 `cdhash`。当前 staged app 是 ad-hoc 签名，`codesign -dr - dist/WalkFlow-Mac.app` 只给出 `designated => cdhash H"f98a3fb31a3837ef001bf2438b98173140eaf071"`，且本机 `security find-identity -p codesigning -v` 返回 `0 valid identities found`。结论：Accessibility 授权记录绑定旧 ad-hoc code requirement，重新 build/stage 后当前 App 不再匹配，`AXIsProcessTrusted()` 仍为 false。
  - Accessibility TCC mismatch RED：新增 `BuildRunScriptTests.testLaunchExistingModeDoesNotRebuildOrRestageBeforeModeDispatch` 后，focused test 失败；失败点证明 `script/build_and_run.sh` 缺少 `--launch-existing`，且旧脚本在 `case "$MODE"` 前无条件 `build_app` / `stage_bundle`，导致授权后再次启动也会改变 `.app` bits。
  - Accessibility TCC mismatch GREEN：`script/build_and_run.sh` 新增 `--launch-existing|launch-existing`，仅执行 `stop_app` 和打开现有 `dist/WalkFlow-Mac.app`；`run`、`--verify`、`--logs`、`--telemetry` 仍 build/stage。focused test 已通过，证明 no-build launch 入口存在且 mode dispatch 前不再无条件 build/stage。
  - Accessibility TCC mismatch code-review：reviewer 未发现 Critical；发现 1 个 Important 和 1 个 Minor。Important：原 no-build 回归测试没有直接检查 `--launch-existing` branch 不能调用 build/stage，也没有正向检查 rebuild modes 仍 build/stage。Minor：历史文档仍写 Accessibility 现场阻塞已解除，容易误导后续接手。
  - Accessibility TCC mismatch review fixes：已增强 `BuildRunScriptTests`，直接解析 `--launch-existing` 分支并断言不含 `build_and_stage_app` / `build_app` / `stage_bundle`，同时新增 rebuild modes 仍 build/stage 的正向测试；补充 `debug` 分支先 `stop_app` 的回归测试。历史文档口径已修正为 Accessibility UI 授权确认完成但 runtime trust 仍因 TCC code requirement mismatch 未解除。
  - Accessibility TCC mismatch final verification：`swift test --filter BuildRunScriptTests` 通过，3 个 XCTest，0 failures；full `swift test` 通过，93 个 XCTest，0 failures；`swift build` 通过；`bash -n script/build_and_run.sh` 通过；`git diff --check` 通过；`./script/build_and_run.sh --verify` 通过，输出 `Verified WalkFlowMac is running.`；冻结 `plan.md` hash 仍为 `418fcbab21b9bcf18be86ff550bd5d1cc754f9a5bbfa71903dc556b257d198d7`；SwiftUI 禁用检查无命中；最终 staged app 的 current designated requirement 为 `cdhash H"f98a3fb31a3837ef001bf2438b98173140eaf071"`。
